...
- **Atributos**:
  ...
  - `Precio total`(Calculado): Precio total del medicamento, es decir, añadiendo impuestos. Ejemplo: `5.50`
  - `Receta_médica`: Indica si el medicamento requiere receta médica para su venta (`booleano`). Ejemplo: `true`

### 2. Laboratorio
- **Descripción**: Representa a los laboratorios que suministran/fabrican algunos de los medicamentos vendidos en la farmacia (la propia farmacia puede fabricar algunos).
- **Atributos**:
  ...
  - `Teléfono`(Multievaluado): Número de teléfono de contacto del laboratorio. Ejemplo: `+34 912345678`
  - `Dirección_Postal`: Dirección completa del laboratorio Calle/Número/Código postal. Ejemplo: `Calle Mayor, 10, 28013`
  - `Fax`(Multievaluado): Número de fax del laboratorio. Ejemplo: `+34 912345679`
...
- **Atributos**:
  ...
  - `Enfermedades`(Multievaluado): Enfermedades o condiciones que trata. Ejemplo: `Dolor de cabeza, catarro...`

### 4. Cliente
- **Descripción**: Representa a los clientes de la **farmacia con crédito**, ya que si no tienen crédito es simplemente una persona que va a comprar a la farmacia y no se almacenan sus datos personales.
- **Atributos**:
  - `DNI`: DNI del cliente. Ejemplo: `79151744V`
  - `Nombre`: Nombre completo del cliente. Ejemplo: `Ana López`
  - `Teléfono`(Multievaluado): Número de teléfono del cliente. Ejemplo: `+34 912345680`
  - `Email`(Multievaluado): Dirección de correo electrónico del cliente. Ejemplo: `ana.lopez@gmail.com`
  - `Fecha de pago`: Fecha en la que paga el cliente cada mes. Ejemplo: `25` (el día 25 de cada mes)
  - `IBAN`: Código IBAN de la cuenta bancaria del cliente. Ejemplo: `ES91 2100 0418 4502 0005 1332`
...
- `Cantidad`: Cantidad de medicamentos de un tipo específico. Ejemplo: `3`
...
2. **Stock Suficiente**: No se puede realizar una venta si la cantidad de unidades en `Stock` son menores que las `Unidades_Vendidas` en una transacción.
...
4. Un medicamento solo puede pertenecer a una familia: Un medicamento está asociado con una única familia de enfermedades. No puede pertenecer a varias familias simultáneamente.
